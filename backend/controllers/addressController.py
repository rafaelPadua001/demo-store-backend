    ...
    def create_address(user_id, data):
        try:
            ...
            new_address = Address(
                ...
                **{
                    model_field: data.get(front_key)
                    for front_key, model_field in mapping.items()
                    if data.get(front_key) is not None
                }
            )

            required_fields = ['zip', 'street', 'number', 'neighborhood', 'city', 'state', 'country']
    ...
    def update_address(user_id, address_id, data):
        try:
            ...
            for key, value in data.items():
                ...
                if mapped_key and hasattr(address, mapped_key):
